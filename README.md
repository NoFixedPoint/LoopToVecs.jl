...
LoopToVecs.jl is a lightweight package that converts loop-based computations in einsum notation into vectorized code. It exports a single macro, `@t`, with three assignment forms:
- `:=` creates a new variable (e.g., `@t A[i,j] := B[i,j] * 2.0`)
- `=` writes in-place into a pre-allocated array (e.g., `@t A[i,j] = B[i,j] * 2.0`)
- `+=` adds in-place into a pre-allocated array (e.g., `@t A[i,j] += B[i,j]`)

For usage examples, see below and in the `test/` directory.
...
# broadcasting (`:=` creates a new variable)
